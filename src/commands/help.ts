...
function helpCommands(): string {
	return [
		"commands:",
		"",
		"  tala init                       scaffold .tala/config.json + dirs (wizard)",
		'  tala new "Title"                create a new research topic',
		"  tala get                        list resources (topics, tools)",
		"  tala get <resource> [<name>]    inspect a resource",
		"    --claims | --sources | --computations | --full",
		"    --marker <name>               filter claims view",
		"    --status <name>               filter list view",
		"    --format json                 machine-readable output",
		"  tala index                      regenerate the topics index",
		"  tala validate [slug]            validate structure and content",
		'  tala search "query"             full-text search across topics',
		"  tala check                      show staleness report",
		"  tala compute <slug> <script>    run script, capture output",
		"    [-- args...]                  forward args to the script",
		"  tala tool <slug> <name>         run a reusable tool from tools/",
		"    [-- args...]                  forward args to the tool",
		"  tala tools                      alias for `tala get tools`",
		"  tala gather <slug>              output gather context for a topic",
		"  tala verify <slug>              check claims and output corroboration context",
		"  tala refine <slug>              output tournament protocol for a section",
		"    [--section S]                 section to refine (default: Findings)",
		"  tala refine apply <slug>        write refined section back to hub",
		"    --section S                   section that was refined",
		"  tala conventions                regenerate AGENTS.md",
		"  tala help [topic]               show conventions and usage",
		"",
