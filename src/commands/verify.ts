...
import { parseMarkdown, extractWikilinks } from "../parser/markdown"
import { loadTopicCriteria } from "../orchestrator/criteria"
import { verifierCheckPrompt, verifierSearchPrompt } from "../orchestrator/prompts"
...
export async function extractClaimsForVerification(labRoot: string, slug: string): Promise<VerificationClaims> {
	...
	if (Either.isLeft(parsed)) return { withSources, needingCorroboration }
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (wikilinks.length > 0) {
			withSources.push({ text: claim, sourceTarget: wikilinks[0].target, lineNumber: i + 1 })
		}

		if (/\*\(unsupported\)\*/.test(claim)) {
			needingCorroboration.push({ text: claim, marker: "unsupported", lineNumber: i + 1 })
		} else if (/\*\(single-source\)\*/.test(claim)) {
			needingCorroboration.push({ text: claim, marker: "single-source", lineNumber: i + 1 })
		}
	}
...
export async function runVerify(
...
): Promise<{ claimsChecked: number; markersChanged: number; roundsCompleted: number }> {
	let totalChecked = 0
	let totalChanged = 0
	...
	return { claimsChecked: totalChecked, markersChanged: totalChanged, roundsCompleted }
}
