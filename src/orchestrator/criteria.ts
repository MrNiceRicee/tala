...
export const DOMAIN_CRITERIA: Record<string, CriteriaSet> = {
	...
	travel: {
		accuracy: [
			"prices verified with dates noted",
			"seasonal availability flagged",
			"currency and exchange rates cited",
		],
	},
	pharmacy: {
		rigor: [
			"clinical trial references required",
			"dosage claims need official source",
			"drug interaction data from authoritative databases",
		],
	},
};
...
export async function loadTopicCriteria(
	labRoot: string,
	slug: string,
): Promise<string> {
	...
	if (existsSync(hubPath)) {
		...
		if (parsed.ok) {
			const domainValue = parsed.value.data.domain;
	...
	if (existsSync(criteriaPath)) {
		...
		const parsed = parseFrontmatter(raw);
		const content = parsed.ok ? parsed.value.content : raw;
