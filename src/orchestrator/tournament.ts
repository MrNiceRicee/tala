...
export function parseJudgeRanking(output: string): JudgeRanking | null {
	const match = output.match(/RANK:\s*1st\s*=\s*(\d+)\s*,\s*2nd\s*=\s*(\d+)\s*,\s*3rd\s*=\s*(\d+)/)
...
export function computeBordaScores(rankings: JudgeRanking[]): Map<number, number> {
...
export function determineWinner(scores: Map<number, number>, incumbentIdx: number): number {
...
export function shuffleProposals(a: string, b: string, ab: string): { proposals: string[]; indexMap: number[] } {
	const items = [
...
export function runTournament(config: {
	...
	return Effect.gen(function* () {
		...
		for (let round = 1; round <= config.maxPasses; round++) {
			...
			rounds.push({ round, winner: winnerLabel, bordaScores: scores, critique, challenges })
			...
			if (streak >= config.convergenceK) {
				return { converged: true, rounds, finalSection: incumbent, totalRounds: round }
			}
		}

		return { converged: false, rounds, finalSection: incumbent, totalRounds: config.maxPasses }
	})
}
