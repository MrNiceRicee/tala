...
export function buildClaudeArgs(config: {
	...
	const args: string[] = []
	if (config.mode === "print") { args.push("-p") }
	if (config.systemPrompt) { args.push("--append-system-prompt", config.systemPrompt) }
	if (config.input && config.mode === "print") { args.push(config.input) }
	return args
}
...
export function runClaudeAgent(config: {
	...
	return runAgent({ command: "claude", args, mode: config.mode, cwd: config.cwd, visible: config.visible })
}
