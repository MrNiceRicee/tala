...
describe("runComputation", () => {
	it("runs a script and captures output", async () => {
		...
		const scriptPath = join(
			labRoot, "topics", "salary-research", "computations", "add.ts",
		)
		await Bun.write(scriptPath, 'console.log("result: " + (100 + 200))')

		const result = await runComputation(labRoot, "salary-research", "add.ts", [])
		expect(result.success).toBe(true)
		expect(result.output).toContain("result: 300")

		const outputPath = join(
			labRoot, "topics", "salary-research", "computations", "add.output.md",
		)
	...
	it("passes args to the script", async () => {
		...
		const scriptPath = join(
			labRoot, "topics", "args-test", "computations", "echo-args.ts",
		)
		await Bun.write(
			scriptPath,
		...
		const result = await runComputation(
			labRoot, "args-test", "echo-args.ts", ["--salary", "165000"],
		)
		expect(result.success).toBe(true)
	...
	it("fails for nonexistent script", async () => {
		...
		const result = await runComputation(
			labRoot, "missing-script", "nope.ts", [],
		)
