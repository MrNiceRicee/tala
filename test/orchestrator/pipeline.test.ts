...
import { runPipeline, type PipelineStep } from "../../src/orchestrator/pipeline"
...
describe("runPipeline", () => {
		...
		const steps: PipelineStep[] = [
			{
				...
				outputKey: "step1_out",
				run: (artifacts) =>
					Effect.succeed({ output: `processed: ${artifacts.get("input")}`, stderr: "", exitCode: 0 }),
			},
			{
				name: "step2",
				receives: ["step1_out"],
				outputKey: "step2_out",
				run: (artifacts) =>
					Effect.succeed({ output: `final: ${artifacts.get("step1_out")}`, stderr: "", exitCode: 0 }),
			},
		]
	...
	it("handles empty pipeline", async () => {
		const result = await Effect.runPromise(runPipeline([], new Map([["x", "y"]])))
